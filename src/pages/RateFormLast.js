...
import { addRatings, selectRatings, selectPlayerInfo } from '../store/playerInfoSlice';
...
export default function RateFormLast() {
  ...
  const handleZavrsi = (e) => {
    dispatch(addRatings({stage: 'post-blind', zadovoljstvo: zadovoljstvo, opreznost: opreznost}));
    e.preventDefault();
    e.stopPropagation();
    setPopup(true);
  }
  ...
  return (
    <div>
      {popup && 
        <Popup>
          <h3>Prije nego li Vam damo rezultate koji se odnose na Vaš uradak na igrici, zamolili bi Vas da nam odgovorite jesu li Vaši podaci pogodni za istraživanje? Rezultati će Vam se prikazati bez obzira kako odgovorili, stoga molimo da budete iskreni, jer će nam to od velike pomoći.</h3>
      ...
      <div className={styles.container}>
        <h1 className={styles.title}>Ostvarili ste {bodovi} bodova</h1>
          ...
          <Button variant="primary" className="mt-5" name="button" size="lg" disabled={zadovoljstvo === null || opreznost === null} onClick={handleZavrsi}>
            Završi
          </Button>
      </div>
    </div>
